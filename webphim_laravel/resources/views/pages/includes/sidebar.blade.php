       ...
       <section class="tab-content">
          <div role="tabpanel" class="tab-pane active halim-ajax-popular-post">
             ...
             <div id="halim-ajax-popular-post" class="popular-post">
                @foreach ($phimhot_trailer as $key => $trailer_sidebar)
                <div class="item post-37176">
                   <a href="{{ route('movie',$trailer_sidebar->slug) }}" title="{{ $trailer_sidebar->title }}">
                      <div class="item-link">
                         <img src="{{ asset('uploads/movie/'.$trailer_sidebar->image) }}" class="lazy post-thumb" alt="{{ $trailer_sidebar->title }}" title="{{ $trailer_sidebar->title }}" />
                         <span class="is_trailer">
                           @if ($trailer_sidebar->resolution==0)
                           HD
                       @elseif($trailer_sidebar->resolution==1)
                           SD
                       @elseif($trailer_sidebar->resolution==2)
                           HDCam
                       @elseif($trailer_sidebar->resolution==3)
                           Cam
                       @elseif($trailer_sidebar->resolution==4)
                           FullHD
                       @else
                           Trailer
                       @endif
                      ...
                      <p class="title">{{ $trailer_sidebar->title }}</p>
                   </a>
                  ...
                  <div style="float: left;">
                     <ul class="list-inline rating"   title="Average Rating">
                        @for($count=1; $count<=5; $count++)
                              <li title="star_rating" style="cursor:pointer; font-size:20px; padding:0; color:#ffcc00; ">
                                 &#9733;
                              </li>
                        @endfor
                     </ul>
                ...
                <div id="halim-ajax-popular-post" class="popular-post">
                    ...
                    {{-- <div class="item post-37176">
                       <a href="chitiet.php" title="CHỊ MƯỜI BA: BA NGÀY SINH TỬ">
                          <div class="item-link">
                             <img src="{{ asset('/image/1568475588-881-chi-muoi-ba-official-poster-1568431124-width700height1000.jpg') }}" class="lazy post-thumb" alt="CHỊ MƯỜI BA: BA NGÀY SINH TỬ" title="CHỊ MƯỜI BA: BA NGÀY SINH TỬ" />
